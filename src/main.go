...
func parseFlags() {
	...
	flag.StringVar(&targetServer, "targetServer", "", "Target server IP address or domain name (required)")
	flag.StringVar(&targetPort, "targetPort", "", "Target server port (required)")
	...
	if probeJson == "" || targetServer == "" || targetPort == "" {
		fmt.Println("Usage of the trojan-prober:")
		flag.PrintDefaults()
		os.Exit(1)
	}
	serverAddr = net.JoinHostPort(targetServer, targetPort)
}
...
func executeProbe(probe string) (bool, string) {
	...
	if probe == "H1-Close" || probe == "H1-Incomplete" {
		go captureFINPacket("any", targetServer, uint16(portInt), probe)
	}

	// Establish TLS connections
	tlsConn, err := establishTLSConnection(serverAddr, targetServer, probeData.ALPN)
	if err != nil {
...
func establishTLSConnection(addr, serverName, alpn string) (*tls.Conn, error) {
